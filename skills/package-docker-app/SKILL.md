...
7. 用中文展示完整计划和 `plan_hash`。明确列出本地构建镜像、要打包的原始第三方镜像、要复用的服务器镜像引用、端口映射、省略的映射、环境变量值、复制文件和保留的服务器路径；选择 `keep_server_path` 的路径必须明确标记为不进入归档并保留原始 Compose source。要求用户明确确认，不得把用户最初的打包请求视为这次确认。
